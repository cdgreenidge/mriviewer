...
void MriWindow::draw() {
	...
	if (!context_valid()) {
		resources_.vertexShader = makeShader(GL_VERTEX_SHADER,
						    "vertexShader.glsl");
		resources_.fragmentShader = makeShader(GL_FRAGMENT_SHADER,
						      "fragmentShader.glsl");
		resources_.program = makeProgram(resources_.vertexShader,
	...
	resources_.texture = makeTexture(image_.data(), image_.width(),
                                     image_.height());
	...
	GLfloat vertexBufferData[] = {
		...
		-width, crosshairY, 0.0f, 0.0f,
		width, crosshairY, 0.0f, 0.0f,
		/* Vertical crosshair line, with 8 bytes of padding */
	        crosshairX, -height, 0.0f, 0.0f,
		crosshairX, height, 0.0f, 0.0f
	};

	glDeleteBuffers(1, &resources_.vertexBuffer);
	resources_.vertexBuffer = makeBuffer(GL_ARRAY_BUFFER, vertexBufferData,
					    sizeof(vertexBufferData));
	glDeleteBuffers(1, &resources_.elementBuffer);
	static const GLushort elementBufferData[] = { 0, 1, 2, 3, 4, 5, 6, 7,
						      8};
	resources_.elementBuffer = makeBuffer(GL_ARRAY_BUFFER, elementBufferData,
...
GLuint MriWindow::makeTexture(const GLfloat *slice, size_t bufWidth,
	...
	glTexImage2D(
		GL_TEXTURE_2D, 0,
		GL_LUMINANCE,
		static_cast<GLsizei> (bufWidth),
		static_cast<GLsizei> (bufHeight),
		0,
